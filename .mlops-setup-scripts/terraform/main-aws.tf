...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = "mlc_mlops_stacks_aws-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "mlc_mlops_stacks_aws-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for mlc_mlops_stacks_aws"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "mlc_mlops_stacks_aws-terraform-cicd-user"
}
...
resource "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "mlc_mlops_stacks_aws-cicd-setup-tfstate"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "mlc_mlops_stacks_aws-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
