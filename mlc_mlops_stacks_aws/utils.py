...
def _get_ml_config_value(env, key):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"mlc_mlops_stacks_aws have been deployed to {env} (see mlc_mlops_stacks_aws"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"mlc_mlops_stacks_aws-model{resource_name_suffix}"
    else:
        # Read ml model name from mlc_mlops_stacks_aws/terraform
        return _get_ml_config_value(env, "mlc_mlops_stacks_aws_model_name")
